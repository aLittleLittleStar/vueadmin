...
#### 个人中心页面编写
##### 思路： 
考虑到个人中心和个人主页页面信息大部分相同，可以使用vuex进行数据共享 <br>
但是：考虑到进程问题首先用简单的方法：等基本功能完成之后再进行改动， <br>
一共七项：
```bash
+ 个人资料：
  + 修改个人资料
...
```bash
新问题：
更新用户昵称时，因为路由跳转时用到的是用户名，所以路由出现问题，
解决方法：用id替代昵称进行路由跳转
更新用户密码提示出现问题
一堆bug
```
...
#### 项目代码重构
##### 原因
+ 路由是按照昵称进行跳转的，当修改用户昵称后路由失效，出现bug
+ 数据库文章表应该以用户的id进行关联，不然修改用户昵称，还要修改文章表里面用户的昵称

##### 解决
+ 登录时储存用户的id，以id进行路由的跳转

##### 修改
1. 将路由跳转到个人中心换为以id进行跳转

```bash
# 如果要进行修改
1. 数据库article表 articleavatar 要变为 articleid
2. 用户收藏表也要把 username 改为 userid
3. 用户点赞表也要把 username 改为 userid
4. 相对应的前后端各种查找、添加、删除都要把 用户昵称 改为 用户id
5. 改动太多【烦】
...
```bash
另外一种思路：
在更新用户名时：更新文章表、资源表、收藏表、点赞表

```

### 2019/7/8
#### 修复 文章详情页面无样式的bug
使用 `v-html` 进行转译 而不是直接用 `{{}}`
